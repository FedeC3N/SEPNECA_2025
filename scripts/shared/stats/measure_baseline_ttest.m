...
switch config.measure
    ...
    case 'strength'
        strength_baseline_ttest(config,measure,group);
end
...
for iband = 1 : numel(config.bands)
    ...
    for ipair = 1 : size(measure.all,1)
        ...
        [~,p,CI,stats] = ttest2(measure_v1(group==1),measure_v1(group==0));
...
function strength_baseline_ttest(config,measure,group)
...
for iband = 1 : numel(config.bands)
    ...
    for isensor = 1 : size(measure.all,1)
        ...
        % Create vectors for ANOVAN model
        measure_v1 = measure.baseline(isensor,:,iband)';
